...
public class ShowPost {
                ...
                try {
                    ...
                    if (postNumber >= 1 && postNumber <= 5) {
                        ...
                        if (action.equals(MYPAGE)) {
                            ...
                            System.out.println("로그인한다음 내글만 보기 들어가서 해야 삭제또는 수정가능함");
                        }
                    ...
                    System.out.println("숫자또는 next prev만 입력가능");
                }
            }
        }
    }
    ...
    private void printPosts(Post[] posts) {
        ...
        for (int i = 0; i < posts.length; i++) {
            if (posts[i] != null) {
                System.out.println((i + 1) + " (글번호: " + posts[i].getPostIdx() + ")" + "제목 ||" + posts[i].getTitle() +  "날짜 ||" + posts[i].getRegDate() +"글쓴이 ||"+posts[i].getUserID()+"글내용 ||" + posts[i].getText());
            }
        }
    }
    ...
    private void pageing(String nextAction, String action, Member member) {
        if (nextAction.equals("next")) {
            page++;
        } else if (nextAction.equals("prev") && page > 1) {
            page--;
        } else {
